...
assert_keyed_or_ok_or_network_skip() {
  ...
  if [[ "${RESPONSE_STATUS}" == "400" ]]; then
    assert_body_contains 'SERPAPI_API_KEY is not configured' "${label}"
...
echo "Running Google Scholar provider tests against: ${BASE_URL}"

call_get "/search_google_scholar?query=${QUERY}&limit=${LIMIT}" "/search_google_scholar"
assert_keyed_or_ok_or_network_skip "/search_google_scholar" "google_scholar_serpapi"

call_get "/search_scholar?query=${QUERY}&provider=google_scholar_serpapi&limit=${LIMIT}" "/search_scholar provider=google_scholar_serpapi"
assert_keyed_or_ok_or_network_skip "/search_scholar provider=google_scholar_serpapi" "google_scholar_serpapi"

echo "PASS: Google Scholar provider tests"
