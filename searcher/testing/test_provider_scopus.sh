...
QUERY="${QUERY:-aviation+noise+model}"
LIMIT="${LIMIT:-2}"
START="${START:-0}"
...
assert_keyed_or_ok_or_network_skip() {
  ...
  if [[ "${RESPONSE_STATUS}" == "400" ]]; then
    assert_body_contains 'ELSEVIER_API_KEY is not configured' "${label}"
...
echo "Running Scopus provider tests against: ${BASE_URL}"

call_get "/search_scopus?query=${QUERY}&limit=${LIMIT}&start=${START}" "/search_scopus"
assert_keyed_or_ok_or_network_skip "/search_scopus" "scopus"

call_get "/search_scholar?query=${QUERY}&provider=scopus&limit=${LIMIT}&scopus_start=${START}" "/search_scholar provider=scopus"
assert_keyed_or_ok_or_network_skip "/search_scholar provider=scopus" "scopus"

echo "PASS: Scopus provider tests"
